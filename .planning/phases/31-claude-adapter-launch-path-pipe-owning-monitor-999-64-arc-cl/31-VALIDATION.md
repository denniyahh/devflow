...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| TBD — planner fills | — | — | Constraint 1 | — | Adapter always emits `--input-format stream-json --output-format stream-json`; prompt via stdin, not argv | unit | `cargo test -p devflow-core --lib agents::claude::` | ❌ W0 | ⬜ pending |
| TBD — planner fills | — | — | Constraint 4 | — | Close rule fires only on marker-in-top-level-`result` **AND** drained `background_tasks_changed` | unit (fixture-driven, 30a/30c captures) | `cargo test -p devflow-core --lib monitor::` | ❌ W0 | ⬜ pending |
| TBD — planner fills | — | — | Constraints 5/8 | — | Idle timer resets per line; fires at ≥30s clamped floor; writes authoritative result **before** terminating child | unit + torn/coalesced regression fixture | `cargo test -p devflow-core --lib monitor::`, `... agent_result::` | ❌ W0 | ⬜ pending |
| TBD — planner fills | — | — | Constraint 7 | — | `result`-event count is not treated as children-returned; coalesced completions handled | unit, reusing archived coalesced capture | `cargo test -p devflow-core --lib monitor::` | ❌ W0 (fixture data exists) | ⬜ pending |
| TBD — planner fills | — | — | Constraint 9 (residual) | — | A stream-derived `Success` cannot override a contradicting non-zero exit code | unit (truncated capture + non-zero exit file) | `cargo test -p devflow-core --lib agent_result::` | ❌ W0 | ⬜ pending |
