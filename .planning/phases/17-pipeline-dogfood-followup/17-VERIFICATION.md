...
|------|------|---------|----------|--------|
| `crates/devflow-cli/tests/build_provenance.rs` | 22-28 | `build_commit_is_accessible_and_does_not_panic` still asserts nothing (`let _ = commit.len();`) — `17-VALIDATION.md` GAP-4 / `17-REVIEW.md` IN-01, explicitly left unfixed by both the audit and this plan | ℹ️ Info | Not a must-have for any 17-11 truth; test-only, low severity, does not affect the real coverage gap it inflates (row 7 has 3 real covering tests, not 4) — not a blocker |
| `.planning/phases/17-pipeline-dogfood-followup/17-VALIDATION.md` | frontmatter | `status: validated`, `nyquist_compliant: false` — written against the pre-17-11 (`2fed1a6`, "fifth validation re-audit") state; GAP-3 (the exact defect CR-02 names) is now closed in code but this file was not touched by `17-11` (out of its declared `files_modified` scope, per `17-11-SUMMARY.md`'s own "Next Phase Readiness" note) | ℹ️ Info | Documentation lag, not a functional gap — the underlying property GAP-3 flagged as unsampled now has direct regression coverage (Truth 13). A future re-validation pass should re-sample rows 7/8 and flip `nyquist_compliant` back to `true`, but this does not block Phase 17's goal achievement since the code-level truth is independently verified above, not inferred from this stale document |
