...
threats. Every key below is `plan/ID`, which makes 30 distinct threats.
...
| 47-01/T-47-03 | Tampering | supply chain — `insta`, `similar`, `console` | medium | mitigate | `47-RESEARCH.md` § Package Legitimacy Audit (`:557`); at `0afbff5`, `cargo deny check` reported advisories, bans, licenses and sources ok, and `cargo machete` found no unused dependency | closed |
...
| 47-02/T-47-05 | Elevation of Privilege | `code_policy_applies_to_fix_arm` widening to `GapsOnly`/`AuditFix` | high | mitigate | `claude_style_fix_prompts_that_must_not_carry_the_policy_still_omit_it` green | closed |
...
| 47-03/T-47-09 | Elevation of Privilege | the carve-out widening beyond `blocking-human` to package verification | high | mitigate | `package_verification_prohibition_is_unconditional` green; the part not pinned mechanically was reviewed by the operator (`47-UAT.md` tests 14, 18) | closed |
...
| 47-07/T-47-15 | Denial of Service | `test-phase-worktree-guard.sh` bootstrap | medium | mitigate | Repository-local `core.hooksPath` and `commit.gpgsign false` (`scripts/test-phase-worktree-guard.sh:70-71`); harness `passed=13 failed=0` with 7a, 7b and 7c ok | closed |
| 47-07/T-47-16 | Tampering | guard under test (`lint-phase-worktree.sh`) | medium | mitigate | **Partly superseded — see detail below.** The six direct `"$GUARD" --staged` calls remain and the controls pass, but the harness now exports `GIT_CONFIG_GLOBAL=/dev/null GIT_CONFIG_NOSYSTEM=1` (`:49`) ahead of every guard call, which the planned mitigation ruled out | open — below high threshold (non-blocking) |
...
- **What 47-07 planned.** 47-07 isolated the fixture with repository-local config plus clearing two inherited variables. It said "never an exported config override", so the guard would see its normal environment.
- **What review changed.** Review finding WR-01 (`47-REVIEW.md`) found two escapes: a global `core.excludesFile` hiding `.planning/` aborted the harness, and a leaked `GIT_OBJECT_DIRECTORY` escaped the fixture.
  - `67cab06` closed WR-01 by exporting `GIT_CONFIG_GLOBAL=/dev/null GIT_CONFIG_NOSYSTEM=1` for the whole run.
  - `c66cba2` then moved that export below the harness's one query against the real checkout, so CI's global `safe.directory` entry still applies to that query.
  - The review records the trade-off and argues that nulling global config does not bypass the behaviour under test.
- **Why it is not closed.** Every guard call now runs with no global or system config, so the harness never exercises the guard under an inherited config. The guard reads no config itself. It runs four git commands: `rev-parse --show-toplevel`, `worktree list --porcelain`, `symbolic-ref --short HEAD` and `diff --cached --name-only`. Nobody has checked whether an operator's config changes their output in a way the guard mishandles.
- **What would close it.** Either record the residual risk in the Accepted Risks Log below, or amend the 47-07 register's mitigation to describe the isolation that shipped and re-run this audit. It is below the `high` threshold, so it does not block the phase.
...
- **Depth.** Grep depth confirms each mitigation is present in the code or the planning record at HEAD `d0b5699`, whose code is identical to `49bb324`. It does not trace a mitigation end to end, as ASVS 2 or 3 would.
- **Controls.** Controls were run only where the Mitigation column names one.
...
- [x] Accepted risks documented in Accepted Risks Log — 2
- [x] `threats_open: 0` confirmed — one medium threat (47-07/T-47-16) remains open below the `high` threshold
