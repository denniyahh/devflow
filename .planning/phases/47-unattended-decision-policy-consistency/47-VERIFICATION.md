...
| A copied gate declaration could make a failed resume look like a checkpoint | `resume_prompt_does_not_read_as_a_blocking_human_checkpoint` accepts a real `**Gate:**` declaration but rejects the rendered resume prompt and free-form resolved-gate prose. A copied line-leading `**Gate:**` declaration would still read as a checkpoint: the resume prompt's instruction to name the gate in prose is the only mitigation for copying, and whether an agent follows it is Phase 49's to observe. | ✓ VERIFIED at the detector boundary; copying mitigated by wording only |
