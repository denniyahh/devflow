...
requirements-completed: []  # Corrected 2026-07-29 by the validate→code fix loop. Was ["999.25"]; `26-VERIFICATION.md` flagged that as a partial-progress overclaim (⚠️ warning): this plan built 3 of the ~8 primitives 999.25 needs (`push_ref`, `release_tag_state`, `create_signed_release_tag`) and wired none of them to a production caller, so the backlog item is not completed. The `coverage` D1/D2/D3 blocks below remain accurate as plan-level coverage of the primitives this plan did build — verification affirmed that scoping explicitly; only the top-level "this backlog item is done" reading was wrong.
