...
// The tag-signing viability probe (`check_signing_viability` and its SSH/GPG
// helpers, plus the `SigningViability`/`SignProbeOutcome` enums) was REMOVED in
// 999.104: it was capability-only — "can this key sign" — never identity, which
// is what a wrong-key release tag actually needs. `scripts/cut-release.sh` now
// fails loudly when `devflow.releaseSigningKey` is unset, and the identity check
// that matters lives in `scripts/hooks/pre-push`. Do not reintroduce a
// signing-viability probe here; it is dead weight by construction.
