...
| `devflow release --check` | Read-only release-cut preflight: workspace self-pin, develop/main divergence (no `git fetch` — reads already-fetched refs), crates.io publish order, and `gpg.format`-aware tag-signing viability. `--check` and `--execute` are mutually exclusive; a bare `devflow release` (neither flag) is rejected, naming both modes |
| `devflow release --execute --yes-release` | Runs the real release-cut sequence (26-07, `999.25`): version bump + direct push to `origin/develop` (D-01/D-08), the signed release tag, sync `main` back into `develop` (the same code `devflow sync` calls, D-07), and the crates.io publish for each workspace member in publish order (D-04). `--yes-release` must be typed on this invocation — it authorizes the entire sequence as one act and can never be set via `devflow.toml` or an environment variable. Halts cleanly (exit 0) at the `develop`→`main` human gate if `origin/main` does not yet declare the release version — re-run the same command after that PR merges. The pre-gate runs self-pin and publish-order as blocking checks and reports divergence as information only; it never runs the tag-signing viability check (D-10) — the tag step answers that question by running the real command |
| `devflow sync` | Sync `main` back into `develop` after a release — the Rust port of `scripts/sync-main-to-develop.sh` (999.52). Refuses on a dirty working tree or when not checked out on `develop`; fetches `origin main develop` then no-ops if `origin/main` is already an ancestor of `develop`; otherwise merges with `-X ours` (develop's content wins) and refuses to push if the resulting tree differs from develop's pre-merge tree (leaving the local merge commit for inspection, no automatic undo). On success, direct-pushes the merge to `origin/develop` — no PR, no force option. The same `sync_main_to_develop` function this subcommand calls is also the release executor's internal sync step (D-07) |
