...
| `DEVFLOW_CLAUDE_IDLE_TIMEOUT_SECS` | 30 | How long the pipe-owning Claude monitor waits with NO output on the child's stream before recording an `idle_timeout` verdict and terminating it. Clamped to a 30s floor: a lower value is raised and the clamp is printed loudly at `devflow start` (a 12s bound killed a live, healthy run in 2 of 7 measured trials). Every stream line resets the window, so a healthy long-running stage is never bounded by wall clock |
