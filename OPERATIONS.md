...
| `devflow release status <version> [project]` | **Read-only release-cut observer (29a).** Answers the release-cut questions themselves by querying authoritative external sources — currently the signed-tag-on-`origin` question (`git ls-remote --tags` + `gh api .../git/tags/<sha>`). Every observation is three-valued (Present/Absent/Unreachable); a network or tool failure is reported as Unreachable (exits non-zero) and is never conflated with a genuine Absent (exits 0 with a warn icon). Mutates nothing. |
